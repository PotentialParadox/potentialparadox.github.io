...
* THREE Abstract
  ...
  NEXMD is a computational package particularly developped to perform simulations of the photoexcitation and subsequent non-adiabatic electronic and vibrational energy relaxation and redistribution in large multichormophoric conjugated molecules, a process that involves several coupled electronic excited states.
...
PPVO oligimoner are derivatives of PPV that allow tunable emission spectra across the optical spectrum. (cite:sifain2018photoexcited:85,86)
#+end_src
...
*** THREE Conclusion
    ...
    The program has been tested well and helps gain insights into the dynamics of molecules prone to photo-excitations.
    However, simulations in explicit solvents have produced qualitatively different results than in implicit solvent mediums.
...
- Note taken on [2021-02-07 Sun 08:41] \\
  ...
  - Recent works have analized the non-adiabatic behaviors of PPV3NO2  to the study of its na dynamics in implicit solvent. \cite{sifain2018photoexcited}
...
:END:
   #+begin_src latex
   \section{Introduction}
In chapter 4, we applied adiabatic excited state dynamics to PPV3NO2. While this process allowed us to analyze the steady-state absorption and fluorescence properties, it does not allow us to simulate the exciton movement during the process. Charge transfer and the study of donor-acceptor functionality are important when working with light-harvesting devices, LEDs, or any photosensitive process. (cite) A major feature of PPV oligomers is their sensitivity of these non-adiabatic characteristics to the environment. (cite) This allows users to tailor the properties based upon the choice of residues during the creation of the polymer and allows fine-tuning of these properties based on the selection of the solvent. 

In this chapter, we activate the non-adiabatic routines found in NEXMD for SANDER. We integrate non-adiabatic routines found in NEXMD in SANDER to analyze and simulate PPV3-NO2 in explicit solvent. As shown in the previous chapters, the computational cost for a single trajectory is roughly an order of magnitude larger than that for adiabatic dynamics. To have reasonable precision in our results, we need to have around 5 times the number of trajectories within our swarm. (cite) Due to the high computational costs of the following calculations, we limit our analysis to a single solvent. State population decay is known to be sensitive to the solvent dielectric constant and hydrogen bonding sites. (cite) Out of the solvents we tested in the Adiabatic simulations, methanol seems like the appropriate choice. We also choose to include no more than 10 solvents within the QM region. There is very little difference in the results between 10 and 20 in the QM region during the adiabatic dynamics. Also, as will be seen later, including 10 instead of 5 solvents had an insignificant effect and extrapolation to 20 QM seemed appropriate. We show our method to simulate a pulse pump experiment. We analyze the population decay and the same geometric relaxations that we explored in the adiabatic section.
   #+end_src
** THREE Methods [3/3]
   #+begin_src latex
   \section{Simulation Methods}
...
**** THREE Quick Explanation of a pulse pump experiment
    ...
    :END:
    #+begin_src latex
Pump-Probe Spectroscopy is an experimental technique commonly performed in the study of ultrafast excited-state dynamics. (cite) In the case of conjugated polymers, it can help the study of localized excitonic transitions accessible through an excitation from the S1 state but not the ground state S0. (cite)
We extracted the transition dipole frequencies and oscillator strengths from the S1 single-point calculations we ran on the final snapshot of the QM ground state. These are then used to determine the initial excited-state (Sm) of the non-adiabatic trajectories using a Frank-Condon methodology. (cite)
...
Spectras for varying number of solvents included in the QM region and vacuum are included.
...
#+begin_src latex
Figure \ref{fig:dihedralNon-Adiabatic} presents the torsional angles around d1,d2,d3 (Near) and d4,d5,d6 (Far).
...
#+begin_src latex
The visual analysis of the Widberg bond orders can be found in figure \ref{fig:bondOrderNon-Adiabatic}.
